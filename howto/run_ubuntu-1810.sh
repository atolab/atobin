#Script to generate the deb package for ubuntu-18.10
...
wget -O zenohd  https://github.com/atolab/atobin/blob/master/zenoh/latest/ubuntu/18.10/zenohd?raw=true
chmod +x zenohd
mkdir $MAINDIR/zenohd-$VERSION/usr/bin
mv zenohd $MAINDIR/zenohd-$VERSION/usr/bin

wget -O yaksd https://github.com/atolab/atobin/blob/master/yaks/latest/ubuntu/18.10/yaksd?raw=true
chmod +x yaksd
mkdir $MAINDIR/yaksd-$VERSION/usr/bin
mv yaksd $MAINDIR/yaksd-$VERSION/usr/bin


wget -O libzenohc.so https://github.com/atolab/atobin/blob/master/zenoh-c/latest/ubuntu/18.10/libzenohc.so?raw=true 
